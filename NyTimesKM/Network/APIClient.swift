...
extension APIClient {
    ...
    private func decodingTask<T: Decodable>(with request: URLRequest, decodingType: T.Type, completionHandler completion: @escaping JSONTaskCompletionHandler) -> URLSessionTask {
        ...
        let task = decodingTask(with: request, decodingType: T.self) { (json, error) in
            ...
            DispatchQueue.main.async {
                ...
                if let value = decode(json) {
                    ...
                    completion(APIResult.failure(.jsonParsingFailure))
                }
            }
        }
